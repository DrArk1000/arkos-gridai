...
export default function GridAnalyzer() {
  ...
  const analyzeGrid = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Grid analysis failed")
      }
      ...
      setError("Failed to analyze grid connection. Please verify coordinates and try again.")
          ...
          <div className="space-y-4">
            ...
            <div className="bg-slate-900/50 backdrop-blur-sm border border-slate-800 rounded-xl p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-white">Bankability Assessment</h3>
                <TrendingUp className="w-5 h-5 text-blue-400" />
              </div>
              <div
                className={`inline-flex items-center px-4 py-2 rounded-full text-sm font-medium border ${getBankabilityColor(results.bankability_rating)}`}
              >
                {results.bankability_rating.toUpperCase()}
              </div>
